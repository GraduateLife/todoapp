...
const COLOR_OPTIONS: { key: NoteColor | 'random'; label: string; hex: string }[] = [
    ...
    >
      ...
      {!isExpanded && (
        <div style={{ display: 'flex', alignItems: 'stretch', width: '100%' }}>
          ...
          {/* ── Input area ─────────────────────────────────────────────── */}
          <div className="rf-input-inner" style={{ flex: 1, border: 'none', padding: '0 1rem', maxWidth: 'none', margin: 0 }}>
      ...
      {isExpanded && (
        <div style={{ padding: '0 0 0.5rem', width: '100%', display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 0, alignItems: 'stretch' }}>
            ...
            >
              {/* Left col: SYNTAX */}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 8, letterSpacing: '0.18em', color: 'var(--rf-cyan)', opacity: 0.6, marginBottom: 3 }}>SYNTAX</div>
                {[
                  ...
                  <div key={l.text} style={{ fontSize: 9, letterSpacing: '0.04em', lineHeight: 1.75, color: l.color, opacity: l.op, whiteSpace: 'pre' }}>{l.text}</div>
                ))}
              </div>

              {/* Right col: PRIORITY + COLOR */}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 8, letterSpacing: '0.18em', color: 'var(--rf-purple)', opacity: 0.6, marginBottom: 3 }}>PRIORITY</div>
                {[
                  { text: '[!+] high',   color: 'var(--rf-purple)', op: 0.75 },
                  { text: '[!]  normal', color: 'var(--rf-purple)', op: 0.50 },
                  { text: '[!-] low',    color: 'var(--rf-purple)', op: 0.35 },
                ].map((l) => (
                  <div key={l.text} style={{ fontSize: 9, letterSpacing: '0.04em', lineHeight: 1.75, color: l.color, opacity: l.op, whiteSpace: 'pre' }}>{l.text}</div>
                ))}

                <div style={{ fontSize: 8, letterSpacing: '0.18em', color: 'var(--rf-amber)', opacity: 0.6, marginTop: 6, marginBottom: 3 }}>COLOR</div>
                {COLOR_OPTIONS.filter((o) => o.key !== 'random').map((opt) => (
                  <div key={opt.key} style={{ display: 'flex', alignItems: 'center', gap: 4, lineHeight: 1.75 }}>
                    <span style={{ width: 5, height: 5, borderRadius: 1, background: opt.hex, display: 'block', flexShrink: 0, opacity: 0.85 }} />
                    <span style={{ fontSize: 9, letterSpacing: '0.04em', color: opt.hex, opacity: 0.65, fontFamily: 'inherit' }}>
                      [{opt.key}]
                    </span>
                  </div>
                ))}
            ...
            <div style={{ flex: 1, padding: '0.2rem 0 0 0.25rem', display: 'flex', flexDirection: 'column' }}>
          ...
          >
            ...
            <div className="flex items-center gap-2 flex-shrink-0" style={{ paddingRight: '0.5rem' }}>
              {attachments && (
